...
// Set subpage
$subpage = ( ! empty( $_GET['subpage'] ) ? str_replace( '_subscriber', '', sanitize_text_field( $_GET['subpage'] ) ) : '' );
...
<div class="wrap oih-wrap oih-wrap-<?php echo $subpage ?>-subscriber">
	...
	<form method="POST" action="">
		...
		<h1>
			<?php
				...
				if( ! empty( $list_id ) ) {
					...
					$list 	  = oih_get_list( $list_id );
					$list_url = add_query_arg( array( 'subpage' => 'view_subscribers', 'list_id' => $list_id ), $this->admin_url );

					// List details
					echo '<div class="oih-heading-list-details">';

						echo '<span>List #' . esc_html( $list_id ) . ' - </span>';
						echo '<a href="' . $list_url . '">' . $list->get('name') . '</a>';
		...
		<div class="oih-opt-in-settings-panel-wrapper">
			...
			<div class="oih-opt-in-settings-footer">
				...
				<?php 
					if( $subpage == 'edit' )
						echo '<a class="oih-trash" onclick="return confirm( \'' . __( "Are you sure you want to delete this subscriber?", "opt-in-hound" ) . ' \' )" href="' . wp_nonce_url( add_query_arg( array( 'page' => 'oih-lists', 'subscriber_id' => $subscriber->get('id') ) , admin_url( 'admin.php' ) ), 'oih_delete_subscriber', 'oih_token' ) . '">' . __( 'Delete Subscriber', 'opt-in-hound' ) . '</a>';
				?>
